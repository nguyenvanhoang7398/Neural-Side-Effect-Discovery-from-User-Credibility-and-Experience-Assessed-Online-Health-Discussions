...
We propose a NEural Architecture-based Treatment side effect prediction (NEAT), which is a novel self-supervised prediction task of mentioned drugs' side effects at the discussion level that addresses these shortcomings. 
...
\parttitle{Results}
Evaluations via ablation study show that credibility learning and experience-based attention improves performance in drug side effect discovery from online health discussion. Additionally, output credibility scores statistically correlate with trustworthiness signals, such as number of ``thanks'' received by other forum members. Experience-based attention highlights critical phrases such as symptoms or drugs and enhances ADR extraction models based on sequence labelling.
...
\item A \textit{thread t} %({\it cf}. Table~\ref{sample_thread}) 
